...
```{r}
# DSN leg
s3_table_dsn <- stable_dsn_acc_rnk %>%
...
# DSP leg
s3_table_dsp <- stable_dsp_acc_rnk %>%
...
# SDS leg
s3_table_sds <- stable_sds_acc_rnk %>%
...
# Merge
s3.table.list <- mget(ls(pattern = "s3_table_...")) # make list for row bind
s3_table <- bind_rows(s3.table.list) # combine all 3
s3_table <- replace(s3_table, is.na(s3_table), "No Data") # replace NA with "No Data"

# save table to csv
write_csv(s3_table, "./sup_tables/MS2-S3-Table.csv", col_names = TRUE)
...
# Write to disk
write_csv(stat.all_assays, "./sup_tables/MS2-S4-Table.csv", col_names = TRUE)
```
...
bp.acc <- bp.acc.data %>%
  ...
  geom_text_repel(data=. %>% filter(!is.na(outlier)), aes(label=bcw_id)) +
  geom_jitter(width = 0.2, alpha = 0.3) +
  labs(x = "NIF Group", y = "ACC Concentration (mg/mL)") +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_text(size = 14, face = "bold"),
        axis.text.x = element_text(size = 12, face = "bold"),
        axis.text.y = element_text(size = 12, face = "bold"))
